...
export function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-surface text-body">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/15 bg-[#355d4d]/88 text-white backdrop-blur-md">
        <div className="mx-auto flex h-20 max-w-[90rem] items-center justify-between px-5 lg:px-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <a href="#visita" className="hidden min-h-11 items-center bg-white px-5 text-sm font-semibold text-[#355d4d] transition hover:bg-[#dce9e1] sm:inline-flex">{c.visit}</a>
        ...
        {menuOpen && <nav className="border-t border-white/15 bg-[#355d4d] px-6 py-6 lg:hidden">{c.nav.map(([href, label]) => <a key={href} href={href} onClick={() => setMenuOpen(false)} className="block border-b border-white/10 py-4 text-lg">{label}</a>)}</nav>}
      </header>

      <main>
        <section id="inicio" className="relative min-h-[100svh] overflow-hidden bg-[#355d4d]">
        ...
        <section className="bg-[#496f60] px-5 py-28 text-white lg:px-10 lg:py-40">
